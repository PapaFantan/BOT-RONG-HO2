...
min_diff = st.sidebar.number_input("Độ lệch tối thiểu để đánh",1,10,1)
min_sample = st.sidebar.number_input("Số mẫu tối thiểu",1,20,1)
...
# ---------- Render hàng nút ngang ----------
def render_row(cards, prefix):
    cols = st.columns(len(cards))
    selected = None
    for i, c in enumerate(cards):
        with cols[i]:
            if st.button(c, key=f"{prefix}_{c}"):
                selected = c
    return selected
...
if selected_r and selected_h:
    ...
    if order.index(selected_r) > order.index(selected_h): result="R"
    elif order.index(selected_r) < order.index(selected_h): result="H"
    else: result="T"
...
if col1.button("Undo") and st.session_state.history:
    st.session_state.history.pop(); save_history(st.session_state.history)
...
if len(st.session_state.history)>=2:
    ...
    for i in range(len(st.session_state.history)-1):
        ...
        if (d,t)==target:
            ...
            if next_result!="T": follow_results.append(next_result)
    total=len(follow_results)
    r_count=follow_results.count("R"); h_count=follow_results.count("H")
    ...
    if total>=min_sample:
        ...
        else: st.warning("Bỏ qua (chưa đủ lệch)")
    else: st.warning("Bỏ qua (chưa đủ mẫu)")
